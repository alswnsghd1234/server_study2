...
public class BoardUpdateController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			//1_2 전달된 파일을 저장시킬 서버의 물리적인 경로 알아내기 (String savePath)
			...
			if(multiRequest.getOriginalFileName("reUpfile") != null) {
				...
				if(multiRequest.getParameter("originFileNo")!=null) {
					...
					//Update Attachment 원래 있던 데이터에 새로운 파일 이름과 변경된 파일이름을 넣어서 변경
					newAttachment.setFileNo(originFileNo);
					//기존 파일 삭제 (updateForm에서 hidden으로 넘긴 파일이름으로 삭제)
					...
					//Insert Attachment
					//+현재 게시글 번호
					newAttachment.setRefNo(boardNo);
				}
				
			}
			
			int result = new BoardService().updateBoard(b,newAttachment);
